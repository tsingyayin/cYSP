...
YSPCore文件夹。与此同时，此文件夹内应该含有和其同名的头文件，例如YSPCore.h。
此头文件应该包含此文件夹内的所有头文件，以便在使用YSPCore时，只需要包含此文件即可。
例如下文中的#include "YSPCore/YSPCore.h"。
遵循此规则时，您可以使用Visindigo提供的开发工具快速导出dll和所有相关文件并生成发行文件包。
*/
#pragma execution_character_set("utf-8") //所有含有const char的cpp文件都需要这一句（大概）
#include <VICore>
#include <VIWidgets>
#include "YSPCore/YSPCore.h"

#ifndef YSPCore_DEVELOPER //用于dll导出时不编译主函数，如果你不知道这是什么，请不要修改
visindigo Program {
