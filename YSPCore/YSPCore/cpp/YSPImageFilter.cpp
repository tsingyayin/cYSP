...
void YSPImageFilter::filterOperation(FilterName name, QImage& rawImage, float p_a, float p_b, float p_c, float p_d) {
...
void YSPImageFilter::topFadeCover(QImage* image, float endP) {
	if (endP < 0) { endP = 0; }
	if (endP > 1) { endP = 1; }
...
void YSPImageFilter::grayScale(QImage* image) {
...
void YSPImageFilter::blackCover(QImage* image) {
...
void YSPImageFilter::changeSaturation(QImage* image, float saturation) {
...
void YSPImageFilter::changeLightness(QImage* image, float lightness) {
...
void YSPImageFilter::mirror(QImage* image, float direction) {
	int d = direction;
	if (d != 0 && d != 1 && d != 2) { d = 0; }
	QImage t = image->mirrored(d == 0 || d == 2, d == 1 || d == 2);
	*image = t;
}
void YSPImageFilter::gaussianBlur(QImage* image, int d, bool considerAlpha) {
	if (d % 2 == 0) { d++; }
...
void YSPImageFilter::edgeExtension(QImage* image, int d) {
	*image = *_edgeExtension(image, d);
}
QImage* YSPImageFilter::_edgeExtension(QImage* image, int d) {
	if (d % 2 == 0) { d++; }
	int r = (d - 1) / 2;
	int d2 = d * d;
...
void YSPImageFilter::reverse(QImage* image) {
