...
class SpeakMain :public QWidget
{
	...
	public:
		...
		SpeakMain(QWidget *parent = Q_NULLPTR) {
			...
			FilterList.append({ "无特效","0" });
			FilterList.append({ "上隐藏","1" });
			FilterList.append({ "褪色","2" });
			FilterList.append({ "褪色上隐藏","3" });
			FilterList.append({ "黑白","4" });
			FilterList.append({ "黑白上隐藏","5" });

			FlipList.append({ "不翻转","0" });
			FlipList.append({ "翻转","1" });
